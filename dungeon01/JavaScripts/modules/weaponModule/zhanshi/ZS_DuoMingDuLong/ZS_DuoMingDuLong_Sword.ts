...
import { GA_Trigger_Weapon_DuoMingDuLong_Poison } from "./GA_Trigger_Weapon_DuoMingDuLong_Poison";

@MPlugin()
@registerWeapon(1053)
export class ZS_DuoMingDuLong_Sword extends WeaponBase{
    wid: number = 1053;
    refesh(): void {
    ...
    effect1Desc: string = "伤害增加3%"
    excuteEffet1(): void {
        this.ownerAttribute.addDamage(0.03);
    }
    unExcuteEffet1(): void {
        this.ownerAttribute.reduceDamage(0.03);
    }
    useEffet2: boolean = true;
    effect2Desc: string = "攻击时，有5%概率使敌人进入中毒状态"
    excuteEffet2(): void {
        let asc = this.owner.character.getComponent(AbilitySystemComponent);
        if(asc){
            asc.giveAbility(GA_Trigger_Weapon_DuoMingDuLong_Poison);
        }
    }
    unExcuteEffet2(): void {
        let asc = this.owner.character.getComponent(AbilitySystemComponent);
        if(asc){
            asc.removeAbility(GA_Trigger_Weapon_DuoMingDuLong_Poison);
        }
    }
