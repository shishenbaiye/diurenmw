...
@MPlugin()
export abstract class ConsumableBase extends MObject{
    /**持有者 */
    owner:Player;
    /**持有者属性 */
    ...
    initByData(data:ConsumableData):void{
        this.uuid = data.uuid;
        this.id = data.id;
    ...
    init(){
        let ConsumableConfig = GameConfig.ConsumablesObj.getElement(this.id);
        this.quality = ConsumableConfig.quality;
    }
    ...
    //生成持久化数据体
    getData():ConsumableData{
        let data = new ConsumableData();
        data.uuid = this.uuid;
        data.id = this.id;
        data.num = this.num;
