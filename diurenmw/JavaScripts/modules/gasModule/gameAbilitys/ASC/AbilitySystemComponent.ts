...
export class AbilitySystemComponent extends Script {
    ...
    /**ASC准备好Action */
    public isReady: Action1<Character> = new Action1<Character>();
    //#region 初始化
    private async init() {
        if (SystemUtil.isServer()) {
            if (!this._gameTag) {
                this._gameTag = this.gameObject.addComponent(GameTags);
                this._gameTag.isReady.add(() => {
                    this.isAllTagReady("GameTags");
                    this._gameTag.isReady.clear();
                    this._gameTag.isReady = null;
                })
            ...
            if (!this._blockTag) {
                ...
                this._blockTag.isReady.add(() => {
                    this.isAllTagReady("BlockTags");
                    this._blockTag.isReady.clear();
                    this._blockTag.isReady = null;
                })
        ...
        this.gameTag.addTagAction.add((tag) => {
            if (this.triggerOwnedTagAdded.has(tag)) {
                this.triggerOwnedTagAdded.get(tag).forEach((value) => {
                    this.tryActiveGameAbilityByClass(value);
                })
            }
    ...
    private _isReadyGameTag: boolean = false;
    private _isReadyBlockTag: boolean = false;

    private isAllTagReady(name:string){
        if(name === "GameTags"){
            this._isReadyGameTag = true;
        }
        if(name === "BlockTags"){
            this._isReadyBlockTag = true;
        }
        if(this._isReadyGameTag && this._isReadyBlockTag){
            this.isReady.call(this.gameObject as Character);
            this.isReady.clear();
            this.isReady = null;
        }
    }
    ...
    /**尝试释放一个技能 (类型)*/
    tryActiveGameAbilityByClass(ability:Constructor<GameAbility>): GameAbility {
        if(!ability) {
            this.log.error(`技能参数为空`);
            return null;
        }
        let abilityName = '';
    ...
    /**尝试释放一个技能 (名字)*/
    tryActiveGameAbilityByName(ability: string): boolean {
        if(!ability){
            this.log.error(`技能名字为空`);
            return false;
        }
    ...
    /**应用效果（实例） */
    applyGameEffectInstanceToTarget(gameEffect: GameEffect, target: GameObject): GameEffect {
        let targetAsc = target.getComponent<AbilitySystemComponent>(AbilitySystemComponent);
        if (!targetAsc) {
            this.log.error(`目标没有ASC组件，无法应用效果`);
            return;
        }
        gameEffect.geContext = { sourceASC: this, targetASC: targetAsc, sourceEffect: gameEffect };
        gameEffect.init();
        if (gameEffect.canApply()) {
            targetAsc.gameEffect.push(gameEffect);
            gameEffect.active()
            return gameEffect;
        } else {
            this.log.error(`无法应用效果`);
    ...
    /**应用效果给自己（实例） */
    applyGameEffectInstanceToSelf(gameEffect: GameEffect): GameEffect {
        gameEffect.geContext = { sourceASC: this, targetASC: this, sourceEffect: gameEffect };
        gameEffect.init();
        if (gameEffect.canApply()) {
            this.gameEffect.push(gameEffect);
            gameEffect.active();
            return gameEffect;
        }
        return null;
    ...
    makeOutGoingGameEffect(gameEffect: Constructor<GameEffect>): GameEffect {
        let gameEffectInstance = MFramework.createObject<GameEffect>(gameEffect);
        gameEffectInstance.geContext = { sourceASC: this, targetASC: null, sourceEffect: gameEffectInstance };
        return gameEffectInstance;
    }
