...
declare namespace mw {
    ...
    class AccountService {
        ...
        /**
         * @description 向233发起addFriend并获得回调
         * @effect  调用端生效
         * @precautions 只在移动端由233拉起生效
         * @param resp usage:233的回调
...
declare namespace mw {
     ...
     * @groups 基础类型
     * @instance
     * @description 支持各端的通信，233、引擎、Web和游戏项目可以互相直接进行业务上的消息传递，无需修改引擎代码
     * @networkStatus usage: 客户端
...
declare namespace mw {
    ...
    class PurchaseService {
         ...
         * @precautions 暂未考虑单机模式，第一次结果会通过异步回调的方式返回（构造时自动请求），后续会通过缓存直接执行回调函数（同步）。老版本的233乐园可能出现无回调的情况，需要自行判断超时
         * @example
         ...
         * @precautions 暂未考虑单机模式，只在支持大会员的 233/Playza/口袋方舟 内使用时生效
         * @example
        ...
        /**
         * @groups 服务/货币
         * @description 获取Ark币余额
         * @effect 只在客户端调用生效
         * @precautions 只在支持IAP的233/Playza/口袋方舟内使用时生效
         * @example
         ...
         * @effect 只在客户端调用生效
         * @precautions 只在支持 IAP 的 233/Playza/口袋方舟 内使用时生效
         * @returns Ark币余额更新的委托
         */
...
declare namespace mw {
    /**
     * @description 233中窗口刷新的消息格式
     * @groups 基础类型
     */
    type OnViewRefreshed = () => void;
    /**
     * @description 233中窗口显示模式切换的消息格式
     * @groups 基础类型
...
declare namespace mw {
    ...
    class TeleportService {
         ...
         * @effect 只在服务端调用生效
         * @param sceneName usage:要传送的目标场景名称 default:  range: 依据场景名称而定
         * @param userIds usage:要传送的玩家 userId 数组 default:  range: 数组大小自定义
         * @param options usage:可选的额外传送信息 default:undefined
         * @returns Promise<TeleportResult>，本次请求正常则返回resolve，异常则返回reject
         * @example
         ...
         * @effect 只在服务端调用生效
         * @param roomId usage:要传送的目标游戏Id default:undefined  range: 依据 roomid 的长度决定
         * @param userIds usage:要传送的玩家userId数组 default:undefined  range: 依据 roomid 的长度决定
         ...
         * @effect 调用端生效
         * @param userId usage:目标玩家的 userId  range: 依据 userId 的长度而定
         * @returns 指定玩家所在的房间信息
         * @example
         ...
         * @effect 只在服务端调用生效
         * @param teleportId usage:要查询的传送 Id default:  range:依据 teleportId 长度而定的
         * @returns 玩家传送的来源信息
         * @example
         * 使用示例:创建一个名为"ServerScript"的脚本，放在场景中。代码如下：
         * ```
         ...
         * @effect 只在服务端调用生效
         * @param teleportId usage:要查询的传送Id default:  range: 依据 teleportId 的长度决定
         * @returns 传送时携带的数据
         * @example
         * 使用示例:创建一个名为"ServerScript"的脚本，放在场景中。代码如下：
         * ```
        ...
        /**
         * @description 传送类型变为字符串
         */
        private static teleportTypeToString;
    }
}
