declare namespace mw {
    ...
    class AvatarSettings {
        /**
         * @groups 设置
         * @description 开启或关闭角色优化:通过动态限制骨架网格体组件的tick频率来限制运行动画数据的时间。目的是使用固定的 CPU 预算（不超过它），
         * 同时以最小的系统开销最大限度地提高感知动画质量。系统确定每个平台的固定预算（在游戏线程上执行的工作的毫秒数 (ms)），并确定它是否可以完成所有需要的工作，
         * 或者是否需要减少工作量完成。如果需要减少它，它会根据重要性这样做，并针对几个区域，目标是动态调整负载以适应固定（游戏线程）预算。其中的领域包括：
        ...
        /**
         * @groups 设置
         * @description 开启关闭一个角色优化:通过动态限制骨架网格体组件的tick频率来限制运行动画数据的时间。目的是使用固定的 CPU 预算（不超过它），
         * 同时以最小的系统开销最大限度地提高感知动画质量。系统确定每个平台的固定预算（在游戏线程上执行的工作的毫秒数 (ms)），并确定它是否可以完成所有需要的工作，
         * 或者是否需要减少工作量完成。如果需要减少它，它会根据重要性这样做，并针对几个区域，目标是动态调整负载以适应固定（游戏线程）预算。其中的领域包括：
         * 停止打勾并使用Master Pose Component。
         * 以较低的速率执行更新。
